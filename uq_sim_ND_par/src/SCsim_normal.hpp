	...
	virtual void simulation(const vec2d_float_t& pre_proc_result) const
	{
		...
		{	
			...
			{	
				...
				for(size_t i = 0; i < grid_storage_size; ++i) 
				{
					alpha_xdisp[i] = alpha_xdisp_temp[i]*multi_orthogonal_poly(pre_proc_result[i], k)*multi_normal_distr(pre_proc_result[i]);
					alpha_force0[i] = alpha_force0_temp[i]*multi_orthogonal_poly(pre_proc_result[i], k)*multi_normal_distr(pre_proc_result[i]);
					alpha_force1[i] = alpha_force1_temp[i]*multi_orthogonal_poly(pre_proc_result[i], k)*multi_normal_distr(pre_proc_result[i]);
				}
