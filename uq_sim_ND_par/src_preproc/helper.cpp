	...
	if(stream) 
	{
		while (!feof(stream))
		{
			if (fgets(buffer, sizeof(buffer), stream) != NULL)
			{
				...
				temp >> no_valid_lines_str >> sum_disp_x_str >> sum_forces_x_str >> sum_forces_y_str >> sum2_disp_x_str >> sum2_forces_x_str >> sum2_forces_y_str;
	...
	const double& var_forces_y)
{
	...
	stat_file.open(stat_file_name.c_str(), std::ios::out);
	...
	stat_file << "The mean of the displacement on the x axis is: " << mean_disp_x << std::endl;
	stat_file << "The variance of the displacement on the x axis is: " << var_disp_x << std::endl;
	stat_file << std::endl;
	stat_file << "The mean of force x is: " << mean_forces_x << std::endl;
	stat_file << "The variance of the force x  is: " << var_forces_x << std::endl;
	stat_file << std::endl;
	stat_file << "The mean of force y is: " << mean_forces_y << std::endl;
	stat_file << "The variance of the force y  is: " << var_forces_y << std::endl;
	...
	double& rho_s_p2)
{
	...
	while(getline(config_file, line))
	{
		if(line.at(0) != '#')
		{
			...
			else if(token_1.compare("prob_dim") == 0)
			{
				prob_dim = str_to_number<unsigned int>(token_3);
			}
			else if(token_1.compare("sg_level") == 0)
			{
				sg_level = str_to_number<unsigned int>(token_3);
			}
