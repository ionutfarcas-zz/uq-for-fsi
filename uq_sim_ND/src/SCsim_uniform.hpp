...
public:
	...
	virtual void simulation(const vec2d_float_t& pre_proc_result) const
	{
		...
		{	
			...
			temp_nu_f = pre_proc_result[i][0]*nu_f_p2 + nu_f_p1;
			assert(temp_nu_f >= 0);
			modify_nastin_data = run_insert_nastin_1d(insert_nastin_exec, nastin_dat, temp_nu_f);

			temp_rho_s = pre_proc_result[i][1]*rho_s_p2 + rho_s_p1;
			assert(temp_rho_s >= 0);
		...
		for(int timestep = 0 ; timestep < no_of_timesteps ; ++timestep)
		{
			...
			{	
				...
				for(size_t i = 0; i < grid_storage_size; ++i) 
				{
					alpha_xdisp[i] = disp_x_all[i][timestep]*multi_orthogonal_poly(pre_proc_result[i], k);
					alpha_force0[i] = force0_all[i][timestep]*multi_orthogonal_poly(pre_proc_result[i], k);
					alpha_force1[i] = force1_all[i][timestep]*multi_orthogonal_poly(pre_proc_result[i], k);
				}
