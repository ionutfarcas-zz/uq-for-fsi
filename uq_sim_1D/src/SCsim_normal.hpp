	...
	virtual void simulation(std::vector<double>& pre_proc_result) const
	{
		...
		for(int timestep = 0 ; timestep < no_of_timesteps ; ++timestep)
		{
			for(int j = 0 ; j < ncoeff ; ++j)
			{
				...
				for(int i = 0; i < quad_degree; ++i)
				{
					temp_disp_x += disp_x_all[i][timestep]*ghq.orthogonal_poly(j, sqrt(2)*pre_proc_result[i]) * pre_proc_result[quad_degree + i]/sqrt(M_PI);
					temp_force0 += force0_all[i][timestep]*ghq.orthogonal_poly(j, sqrt(2)*pre_proc_result[i]) * pre_proc_result[quad_degree + i]/sqrt(M_PI);
					temp_force1 += force1_all[i][timestep]*ghq.orthogonal_poly(j, sqrt(2)*pre_proc_result[i]) * pre_proc_result[quad_degree + i]/sqrt(M_PI);
				}
