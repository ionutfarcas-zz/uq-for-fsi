...
public:
	...
	virtual void pre_processing(vec2d_double& nodes, vec2d_double& weights) const
	{
		...
		ghq.quad_nodes_weights(quad_degree, weights_1D, nodes_1D);
	...
	virtual void simulation(vec2d_double& nodes, vec2d_double& weights) const
	{
		...
		for(int timestep = 0 ; timestep < no_of_timesteps ; ++timestep)
		{
			for(int j = 0 ; j < ncoeff_2dim ; ++j)
			{
				...
				for (int i = 0; i < quad_degree_2D; ++i)
				{	
					disp_x += disp_x_all[i][timestep] * multi_orthogonal_poly(sqrt(2.0)*nodes[0][i], sqrt(2.0)*nodes[1][i], j) * weights[0][i] * weights[1][i]/M_PI;
					force0 += force0_all[i][timestep] * multi_orthogonal_poly(sqrt(2.0)*nodes[0][i], sqrt(2.0)*nodes[1][i], j) * weights[0][i] * weights[1][i]/M_PI;
					force1 += force1_all[i][timestep] * multi_orthogonal_poly(sqrt(2.0)*nodes[0][i], sqrt(2.0)*nodes[1][i], j) * weights[0][i] * weights[1][i]/M_PI;	
				}
