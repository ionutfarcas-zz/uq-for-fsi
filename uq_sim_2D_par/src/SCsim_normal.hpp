...
public:
	...
	virtual void simulation(std::vector<double>& pre_proc_result) const
	{
		...
		for (int i = 0; i < local_quad_points; ++i)
		{
			...
			temp_nu_f = sqrt(2.0)*pre_proc_result[global_id]*nu_f_p2 + nu_f_p1;
			assert(temp_nu_f >= 0);
		...
		for(int j = 0 ; j < ncoeff_2dim ; ++j)
		{
			...
			for (int i = 0; i < local_quad_points; ++i)
			{
				...
				{
					temp_disp_x += disp_x_all_2d[i][k] * multi_orthogonal_poly(sqrt(2.0)*pre_proc_result[global_id], sqrt(2.0)*pre_proc_result[k], j) * pre_proc_result[quad_degree + global_id] * pre_proc_result[quad_degree + k]/M_PI;
					temp_force0 += force0_all_2d[i][k] * multi_orthogonal_poly(sqrt(2.0)*pre_proc_result[global_id], sqrt(2.0)*pre_proc_result[k], j) * pre_proc_result[quad_degree + global_id] * pre_proc_result[quad_degree + k]/M_PI;
					temp_force1 += force1_all_2d[i][k] * multi_orthogonal_poly(sqrt(2.0)*pre_proc_result[global_id], sqrt(2.0)*pre_proc_result[k], j) * pre_proc_result[quad_degree + global_id] * pre_proc_result[quad_degree + k]/M_PI;
				}
			}
