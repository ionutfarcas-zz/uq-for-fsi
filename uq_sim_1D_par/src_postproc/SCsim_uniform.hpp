	...
	virtual void simulation(std::vector<double>& pre_proc_result) const
	{
		...
		for(int timestep = 0 ; timestep < no_of_timesteps ; ++timestep)
		{
			for(int j = 0 ; j < ncoeff ; ++j)
			{
				...
				for (int i = 0 ; i < quad_degree ; ++i)
				{
					disp_x += disp_x_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[i] + 0.5) * pre_proc_result[quad_degree + i];
					force0 += force0_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[i] + 0.5) * pre_proc_result[quad_degree + i];
					force1 += force1_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[i] + 0.5) * pre_proc_result[quad_degree + i];
				}

				disp_x = (nu_f_p2 - nu_f_p1)*disp_x/glq.norm_factor(j);
				force0 = (nu_f_p2 - nu_f_p1)*force0/glq.norm_factor(j);
				force1 = (nu_f_p2 - nu_f_p1)*force1/glq.norm_factor(j);
