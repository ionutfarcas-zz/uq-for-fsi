	...
	virtual void simulation(std::vector<double>& pre_proc_result) const
	{
		...
		for(int timestep = 0 ; timestep < local_no_of_timesteps ; ++timestep)
		{
			for(int j = 0 ; j < ncoeff ; ++j)
			{
				...
				for(int i = 0 ; i < local_quad_points ; ++i)
				{
					temp_disp_x += disp_x_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[global_id] + 0.5) * pre_proc_result[quad_degree + global_id];
					temp_force0 += force0_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[global_id] + 0.5) * pre_proc_result[quad_degree + global_id];
					temp_force1 += force1_all[i][timestep]*glq.orthogonal_poly(j, 0.5*pre_proc_result[global_id] + 0.5) * pre_proc_result[quad_degree + global_id];
				}
